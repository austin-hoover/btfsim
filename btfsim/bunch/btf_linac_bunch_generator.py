...
class Base_BunchGenerator(object):
    ...
    def __init__(self, mass=0.939294, charge=-1, ekin=0.0025, curr=40., freq=402.5e+6):
        self.bunch = Bunch()
    ...
    def dump_parmila_file(self, **kwargs):
        ...
        beam_current = (bunch.macroSize()*nParticlesGlobal*self.bunch_frequency*1.0e+3)*(math.fabs(bunch.charge())*consts.charge_electron)
        ...
        part_wave_lenghth = consts.speed_of_light/self.bunch_frequency*bunch.getSyncParticle().beta()
        for i in range(n_particles):
            (x,xp,y,yp,z,dE) = (bunch.x(i),bunch.xp(i),bunch.y(i),bunch.yp(i),bunch.z(i),bunch.dE(i))
            phi = 2*np.pi*(z/part_wave_lenghth + phase_init/360.)
    ...
    def center_bunch(self, bunch):
        ...
        for i in range(nParts):
            ...
            z_avg += bunch.z(i)
            dE_avg += bunch.dE(i)
        (x_avg,xp_avg,y_avg,yp_avg,z_avg,dE_avg) = orbit_mpi.MPI_Allreduce((x_avg,xp_avg,y_avg,yp_avg,z_avg,dE_avg),data_type,mpi_op.MPI_SUM,comm)
        x_avg /= nPartsTotal
        xp_avg /= nPartsTotal
        y_avg /= nPartsTotal
        yp_avg /= nPartsTotal
        ...
        msg = ("Centered bunch, shifted coordinates by: \n"+
               "dx  = %.4f mm, \n"+
               "dy  = %.4f mm, \n"+
               "dxp = %.4f mrad, \n"+
               "dyp = %.4f mrad, \n"+
               "dz  = %.4f mm, \n"+
               "dE  = %.4f keV ")%(x_avg*1e3, y_avg*1e3, xp_avg*1e3, yp_avg*1e3, z_avg*1e3, dE_avg*1e6)
        print(msg)
        return(bunch)        
...
class BTF_Linac_BunchGenerator(Base_BunchGenerator):
    ...
    is in [GeV*m].
    """
    def __init__(self,twissX, twissY, twissZ, mass=0.939294, charge=-1, ekin=0.0025, curr=40., freq=402.5e+6):
    ...
    def get_bunch(self, nParticles = 0, distributorClass = WaterBagDist3D, cut_off = -1.):
        ...
        """
        print("Generating bunch based on Twiss parameters with distributor %s"%distributorClass.__name__)
        ...
        macrosize /= (math.fabs(bunch.charge())*consts.charge_electron)
        distributor = None
        if (distributorClass in [WaterBagDist3D,KVDist3D]):
...
class BTF_Linac_TrPhaseSpace_BunchGenerator(Base_BunchGenerator):
    ...
    The transverse phase spaces are generated by  PhaseSpaceGen class instances
    ...
    """
    def __init__(self, phaseSpGenX, phaseSpGenY, twissZ, mass=0.939294, charge=-1, ekin=0.0025, curr=40., freq=402.5e+6, method = "cdf"):
        """
        ...
        Gives precise number of particles requested (nParticles)
        ...
        Returns slightly fewer particles than requested ( < nParticles)
        """
        self.twissZ = twissZ
    ...
    def get_bunch(self, nParticles = 0, distributorClass = WaterBagDist1D, cut_off = -1.):
        """Returns the pyORBIT bunch with particular number of particles."""
        print("Generating bunch based on X,Y emittance scans with %s method and Z Twiss parameters with distributor %s"%(self.method,distributorClass.__name__))
        ...
        distributor = None
        if(distributorClass in [WaterBagDist1D,KVDist1D]):
            distributor = distributorClass(self.twissZ)
        else:
            distributor = distributorClass(self.twissZ, cut_off)		
        bunch.getSyncParticle().time(0.)
        ...
        # -- deposit particles according to Grid-based method
            ...
            for i in range(nParticles):
                ...
                if(i%size == rank):
                    bunch.addParticle(x,xp,y,yp,z,dE)
        else: raise ValueError("'%s' is not an available method for transverse 2D bunch generation"%(self.method))
        # ---------------------------------------------------------------------------------------------	
        nParticlesGlobal = bunch.getSizeGlobal()
        bunch.macroSize(macrosize/nParticlesGlobal)
        return bunch
...
class BTF_Linac_6DPhaseSpace_BunchGenerator(Base_BunchGenerator):
    ...
    """
    def __init__(self, phaseSpGenX, phaseSpGenY, phaseSpGenZ, mass=0.939294, charge=-1, ekin=0.0025, curr=40., freq=402.5e+6, method = "cdf"):
        """
        method = 'cdf' (default): Uses cumulative distribution function 
        to pick coordinates for a randomly generated probability. 
        (aka inverse transform sampling)
...
class PhaseSpaceGen:
    ...
    def __init__(self, file_name, x_max = 1.0e+36, xp_max = 1.0e+36, threshold=3e-4):
        ...
        arr_in = np.genfromtxt(file_name,comments='%',delimiter = delimiter,filling_values=0.)
        ...
        self.val_matrix[self.val_matrix < val_peak*threshold] = 0.
        # -- re-normalize to sum
        ...
        #print "debug x_min=",self.x_min," x_max=",self.x_max," xp_min=",self.xp_min," xp_max=",self.xp_max
        #print "debug xp_step=",self.xp_step," nx=",self.nx," nxp=",self.nxp," lenMtrY=",len(val_matrix)," lenMtrX=",len(val_matrix[0])
        #--------------------------------------------------------------
        # -- make 2D grid object for PDF
        self.grid2D = Grid2D(self.nx,self.nxp,self.x_min,self.x_max,self.xp_min,self.xp_max)
        self.x_min_gen = self.x_min
        self.xp_min_gen = self.xp_min
        if(self.x_min_gen < - math.fabs(x_max)): self.x_min_gen = - math.fabs(x_max)
        if(self.xp_min_gen < - math.fabs(xp_max)): self.xp_min_gen = - math.fabs(xp_max)
        self.x_max_gen = self.x_max
        self.xp_max_gen = self.xp_max
        if(self.x_max_gen > math.fabs(x_max)): self.x_max_gen = math.fabs(x_max)
        if(self.xp_max_gen > - math.fabs(xp_max)): self.xp_max_gen = math.fabs(xp_max)		
        #--------------------------------------------------------------
        ...
        self.int_grid2D = Grid2D(self.nx,self.nxp,self.x_min,self.x_max,self.xp_min,self.xp_max)
        for ix in range(self.nx):
            self.int_grid2D.setValue(0.,ix,0)
            for ixp in range(1,self.nxp):
                val_0 = self.int_grid2D.getValueOnGrid(ix,ixp-1)			
                val = (self.grid2D.getValueOnGrid(ix,ixp-1) + self.grid2D.getValueOnGrid(ix,ixp))/2.
                self.int_grid2D.setValue(val + val_0,ix,ixp)
        self.s_int_arr = [0.,]
        for ix in range(1,self.nx):
            val_0 = self.s_int_arr[ix-1]
            val = (self.int_grid2D.getValueOnGrid(ix-1,self.nxp-1) + self.int_grid2D.getValueOnGrid(ix,self.nxp-1))/2.
    ...
    def get_xp(self,coeff,ind_x_0,ind_x_1):
        ...
        while( not abs(ind_xp_1 - ind_xp_0) <= 1 ):
            ...
            if(count > count_max):
                ...
                print "debug s_int_arr=",self.s_int_arr
                for ixp in range(self.nxp):
                    print "debug ix=",ind_x_0," ixp=",ixp," val0=",self.int_grid2D.getValueOnGrid(ind_x_0,ixp)," val1=",self.int_grid2D.getValueOnGrid(ind_x_1,ixp)
                sys.exit(1)
            if( g > val0 and g <= val1 ):
    ...
    def grid_sample(self, nParticles=0): 
        ...
        Ndistr = np.floor(self.pdf*nParticles).astype(int)
        Nparts = Ndistr.sum()
        ...
        x  = (X + (np.random.rand(1,Nparts)-0.5)*2*self.x_step) / 1000.
        xp = (XP + (np.random.rand(1,Nparts)-0.5)*2*self.xp_step ) / 1000.
...
class PhaseSpaceGenZPartial:
    """Generates (z,dE) distribution, using 1D e-profile and 
    assuming Gaussian phase to fit specified emittance and beta function.
    """
    def __init__(self, file_name, twissZ, zdistributor = GaussDist1D, cut_off=-1,threshold=1e-3):
        ...
        dEstep = np.mean(deval[1:] - deval[0:-1])*1e3 # this unit has to be in MeV to give correct cdf
    ...
    def get_z_zp(self):
        ...
        ind01 = norm.cdf(dE,scale=self.dErms) # index of particle from [0,1] distribution
...
def dump_bunch_coordinates(file_name, bunch):
    ...
    for cpu_rank in range(size):
        ...
        for i in range(nParts):
            ...
            if(cpu_rank == rank):
                (x,xp,y,yp,z,dE) = (bunch.x(i),bunch.xp(i),bunch.y(i),bunch.yp(i),bunch.z(i),bunch.dE(i))
            (x,xp,y,yp,z,dE) = orbit_mpi.MPI_Bcast((x,xp,y,yp,z,dE),data_type,cpu_rank,comm)
            if(rank == 0):
                s = " %12.5g  %12.5g    %12.5g    %12.5g    %12.5g  %12.5g "%(x*1000.,xp*1000.,y*1000.,yp*1000.,z*1000.,dE*1000.)
                fl_out.write(s+"\n")
    if(rank == 0):
        fl_out.close()
...
def reverse_bunch_coordinate(bunch,axis_ind):
    ...
    for i in range(nParts):
        (x,xp,y,yp,z,dE) = (bunch.x(i),bunch.xp(i),bunch.y(i),bunch.yp(i),bunch.z(i),bunch.dE(i))
        if(axis_ind == 0): bunch.x(i,-x)
        if(axis_ind == 1): bunch.xp(i,-xp)
        if(axis_ind == 2): bunch.y(i,-y)

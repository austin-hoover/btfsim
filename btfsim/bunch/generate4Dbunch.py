...
xhelpstring = (
    "name of text file containing measured x-xp intensity from 4D emittance GUI"
)
parser.add_argument("-x", "--xscan-file", help=xhelpstring, required=True)
yhelpstring = (
    "name of text file containing measured y-yp intensity from 4D emittance GUI"
)
parser.add_argument("-y", "--yscan-file", help=yhelpstring, required=True)
thelpstring = "threshold (fraction of peak) for noise floor"
parser.add_argument(
    "-t", "--threshold", help=thelpstring, required=False, default=threshold_default
)

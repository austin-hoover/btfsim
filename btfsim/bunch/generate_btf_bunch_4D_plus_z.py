...
from btf_linac_bunch_generator import (
    BTF_Linac_TrPhaseSpace_BunchGenerator,
    PhaseSpaceGen,
)
...
print(" ========= PyORBIT Twiss ===========")
print(
    " alpha beta emitt[mm*MeV] Z= %6.4f %6.4f %6.4f " % (alphaZ, betaZ, emittZ * 1.0e6)
)
...
# -- Create bunch
t0 = time.clock()
bunch_cdf = bunch_gen.getBunch(
    nParticles=nParticles, method="cdf", distributorClassZ=GaussDist1D
)
t1 = time.clock()
bunch_grid = bunch_gen.getBunch(
    nParticles=nParticles, method="grid", distributorClassZ=GaussDist1D
)
t2 = time.clock()

...
"""
class forward_track:
	def __init__(self,ax,bx,ex,ay,by,ey,az,bz,ez,Q01,Q02,Q03,Q04,Q05,Q06,Q07,Q08,Q09,pos_cal,current,Part_Num,sc_sover,elpsN):
		import sys
		import math
		import random
		import time
		...
		from btfsim.genbunch.btf_linac_bunch_generator import BTF_Linac_BunchGenerator
		from btfsim.genbunch.btf_linac_bunch_generator import DumpBunchAccNode
		from btfsim.lattice.Lattice_change_class_BTF import new_lattice
		...
		names = ["MEBT1","MEBT2"]
		latticeFileName = defaults.latticedir + "BTF_lattice.xml"    # Read lattice configuration
		...
		xml_file_name = defaults.latticedir + "btf_lattice_jan2019.xml"
		...
		if (magNumbers ==1): 
			quad_name_dict["MEBT:QH01"].setParam("dB/dr",-float(Q01)/quad_name_dict["MEBT:QH01"].getLength())
		else:
			if (magNumbers ==2):
				quad_name_dict["MEBT:QH01"].setParam("dB/dr",-float(Q01)/quad_name_dict["MEBT:QH01"].getLength())
				quad_name_dict["MEBT:QV02"].setParam("dB/dr",-float(Q02)/quad_name_dict["MEBT:QV02"].getLength())
			else:
				if (magNumbers ==3):
					quad_name_dict["MEBT:QH01"].setParam("dB/dr",-float(Q01)/quad_name_dict["MEBT:QH01"].getLength())
					quad_name_dict["MEBT:QV02"].setParam("dB/dr",-float(Q02)/quad_name_dict["MEBT:QV02"].getLength())
					quad_name_dict["MEBT:QH03"].setParam("dB/dr",-float(Q03)/quad_name_dict["MEBT:QH03"].getLength())
				else:
					if (magNumbers ==4):
						quad_name_dict["MEBT:QH01"].setParam("dB/dr",-float(Q01)/quad_name_dict["MEBT:QH01"].getLength())
						quad_name_dict["MEBT:QV02"].setParam("dB/dr",-float(Q02)/quad_name_dict["MEBT:QV02"].getLength())
						quad_name_dict["MEBT:QH03"].setParam("dB/dr",-float(Q03)/quad_name_dict["MEBT:QH03"].getLength())
						quad_name_dict["MEBT:QV04"].setParam("dB/dr",-float(Q04)/quad_name_dict["MEBT:QV04"].getLength())
					else:
						if (magNumbers ==5):
							quad_name_dict["MEBT:QH01"].setParam("dB/dr",-float(Q01)/quad_name_dict["MEBT:QH01"].getLength())
							quad_name_dict["MEBT:QV02"].setParam("dB/dr",-float(Q02)/quad_name_dict["MEBT:QV02"].getLength())
							quad_name_dict["MEBT:QH03"].setParam("dB/dr",-float(Q03)/quad_name_dict["MEBT:QH03"].getLength())
							quad_name_dict["MEBT:QV04"].setParam("dB/dr",-float(Q04)/quad_name_dict["MEBT:QV04"].getLength())
							quad_name_dict["MEBT:QH05"].setParam("dB/dr",-float(Q05)/quad_name_dict["MEBT:QH05"].getLength())
						else:
							if (magNumbers ==6):
								quad_name_dict["MEBT:QH01"].setParam("dB/dr",-float(Q01)/quad_name_dict["MEBT:QH01"].getLength())
								quad_name_dict["MEBT:QV02"].setParam("dB/dr",-float(Q02)/quad_name_dict["MEBT:QV02"].getLength())
								...
								quad_name_dict["MEBT:QV04"].setParam("dB/dr",-float(Q04)/quad_name_dict["MEBT:QV04"].getLength())
								quad_name_dict["MEBT:QH05"].setParam("dB/dr",-float(Q05)/quad_name_dict["MEBT:QH05"].getLength())
								quad_name_dict["MEBT:QV06"].setParam("dB/dr",-float(Q06)/quad_name_dict["MEBT:QV06"].getLength())
							else:
								if (magNumbers ==7):
									quad_name_dict["MEBT:QH01"].setParam("dB/dr",-float(Q01)/quad_name_dict["MEBT:QH01"].getLength())
									quad_name_dict["MEBT:QV02"].setParam("dB/dr",-float(Q02)/quad_name_dict["MEBT:QV02"].getLength())
	   	...
	   	print "Lattice generation completed!"	
		#-----------------------------------------------------
		...
		# set of uniformly charged ellipses Space Charge    The bigger numbre of ellipse, the more accurate of sapce charge calculation
			nEllipses =  int(elpsN )        #10                                # Ellipse method can be used for the initial estimate, because it calculates faster than FFT method.
			print float(elpsN )
		...
		if (float(ax) == 0.0 and float(bx) == 0.0 and float(ex) == 0.0 and float(ay) == 0.0 and float(by) == 0.0 and float(ey) == 0.0 and \
			...
			bunch_in = Bunch()
			bunch_in.readBunch(defaults.defaultdict["OUTDIR"]+"Input_distributin_external.txt")
			print ("Beam current = %.2f mA")%(float(current)*frequency*float(Part_Num)*1.6021773e-19*1000)

		else:
			peak_current = float(current)
			print ("Beam current = %.2f mA")%peak_current
		...
		bunch_gen.dumpParmilaFile(bunch=bunch_in, phase_init = -0.0, fileName = 	defaults.defaultdict["OUTDIR"] + "Distribution_Input_new.txt")
		print "Bunch Generation completed."
		# print "Beam current is: ", bunch_gen.getBeamCurrent()

		#track through the lattice 
		paramsDict = {"old_pos":-1.,"count":0,"pos_step":0.05}    #"pos_step":0.005

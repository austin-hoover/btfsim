...
class simBTF:
    ...
    def __init__(self, outdir=[]):
        ...
        self.dispersionFlag = (
            1  # if 1 will correct for dispersion in recorded twiss parameters
        )
        self.emitnormFlag = 0  # if 1 will calculated normalized emittances
    ...
    def run(self, **kwargs):
        ...
        if type(start) == float or type(start) == int:
            (
                ...
                zs_startnode,
                ze_startnode,
            ) = self.accLattice.getNodeForPosition(start)
        elif type(start) == str:
        ...
        if type(stop) == float or type(stop) == int:
            ...
            if stop > maxsimlen:
                stop = maxsimlen  # don't run longer than simulation
            (
                stopnode,
                stopnum,
                zs_stopnode,
                ze_stopnode,
            ) = self.accLattice.getNodeForPosition(stop)
        elif type(stop) == str:
        ...
        print(
            "Running simulation from %.4f m to %.4f m (nodes %i to %i)"
            % (zs_startnode, ze_stopnode, startnum, stopnum)
        )
        ...
        if self.bunch_in.getSize() > 1:
            self.tracker = bunchUtil.bunchTrack(
                dispersionFlag=self.dispersionFlag, emitnormFlag=self.emitnormFlag
            )
        elif self.bunch_in.getSize() == 1:
            self.tracker = bunchUtil.spTrack()
    ...
    def reverse(self, **kwargs):
        ...
        print(
            "Running simulation in reverse from %s m to %s." % (str(stop), str(start))
        )
    ...
    def initLattice(
        ...
        optional arguments:
        beamline = ["MEBT1","MEBT2"] name of beamliens to load. Default is to exit of 2nd dipole
    ...
    def initSCnodes(
        self, minlen=0.015, solver="ellipse", nellipse=1, gridmult=6, n_bunches=None
    ):
        """
        Set up SC nodes
    ...
    def initBunch(self, **kwargs):
        ...
        if bunchgenerator == "load":
            defaultbunchfilename = (
                self.defaults.homedir + self.defaults.defaultdict["BUNCH_IN"]
            )

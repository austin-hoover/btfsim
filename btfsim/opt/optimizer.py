...
class Matcher():
	...
	def __init__(self,**kwargs):
		...
		if self.mstatefile:
			if self.xmlfile:
				self.sim.initLattice(beamline=beamline,mstatename=self.mstatefile,xml=self.xmlfile,coeffilename=self.coeffilename)
			else:
				self.sim.initLattice(beamline=beamline,mstatename=self.mstatefile,coeffilename=self.coeffilename)
		else:
			if self.xmlfile:
				self.sim.initLattice(beamline=beamline,xml=self.xmlfile,coeffilename=self.coeffilename)
			else:
		...
		if self.analyticQuads:
			z_step = 0.001
			Replace_Quads_to_OverlappingQuads_Nodes(self.sim.accLattice,\
												z_step, \
	...
	def optimize_to_target(self,x0,target,**kwargs):
		...
		opt_result = minimize(self.opt2target_function, x0, \
							  args=target,\
							  method=method,\
							  bounds=bounds,\
							  options=kwargs)
	...
	def optimize_to_equal(self,x0,**kwargs):
		...
		opt_result = minimize(self.opt2equal_function, x0, \
							  method=method,\
							  bounds=bounds,\
							  options=kwargs)
	...
	def optimize_to_min(self,x0,weights,**kwargs):
		...
		opt_result = minimize(self.minimize_function, x0, args=weights,\
								  method=method,\
								  options=kwargs)
...
class PeriodicMatcher():
	...
	def __init__(self,**kwargs):
		...
		if self.analyticQuads:
			z_step = 0.001
			Replace_Quads_to_OverlappingQuads_Nodes(self.simprd.accLattice,\
													z_step, \
													accSeq_Names = beamline, \
													EngeFunctionFactory = quadfunc)
	...
	def optperiodic_function(self,x,*args):
		...
		self.simprd.initBunch(gen='twiss',nparts=20000,\
							ax=ax0,bx=bx0,\
							ay=ay0,by=by0,\
							az=az0,bz=bz0,ez=ez0,\
							dist='gaussian',current=current)
		...
		f = np.sqrt((ax2-ax1)**2 + (bx2-bx1)**2 + (ay2-ay1)**2 + (by2-by1)**2)

		return f
	
	def optimize_periodic_match(self,twiss0,twissz,current=0.04,nparts=20000,niter=30,plotFlag=False,ftol=0.01):
		az0 = twissz[0]
		bz0 = twissz[1]
		ez0 = twissz[2]
		...
		while f>ftol and i<niter:
			...
			self.simprd.initBunch(gen='twiss',nparts=nparts,\
								ax=ax0,bx=bx0,\
								ay=ay0,by=by0,\
								az=az0,bz=bz0,ez=ez0,\
								dist='gaussian',current=current)
			...
			twiss0 = (np.mean([ax0,ax0,ax2]), np.mean([bx0,bx0,bx2]), np.mean([ay0,ay0,ay2]), np.mean([by0,by0,by2]))
			twiss_hist[i,:] = [np.mean([ax0,ax0,ax2]), np.mean([bx0,bx0,bx2]), np.mean([ay0,ay0,ay2]), np.mean([by0,by0,by2])]
			print(twiss0)
			i+=1
